...
fn parse_item<'a>(tokens: &'a[Tok]) -> ParseResult<'a, StmtItem<'a>> {
    match tokens {
        ...
        [Tok{token: Token::Var, ..}, rest..] => {
            match rest {
                [Tok{token: Token::Ident(id), ..}, rest..]=> {
                    ParseResult::Ok(StmtItem::Var(id), rest)
                }
        ...
        [Tok{token: Token::Def, ..}, rest..] => {
            ...
            ParseResult::Ok(StmtItem::Def(Box::new(parsed_expr)), tokens_after_expr)
        }
        // <expr>
        [Tok{token: _, ..}, ..] => {
            let (parsed_expr, tokens_after_expr) = get_parsed!(parse_expr(tokens));
            ParseResult::Ok(StmtItem::Bare(Box::new(parsed_expr)), tokens_after_expr)
        }
        [] => ParseResult::Err(format!("PARSING FAILURE: Reached end of file but expected a statement"))
    }
}
...
fn parse_item_list<'a>(tokens: &'a[Tok]) -> ParseResult<'a, Vec<StmtItem<'a>>> {
    ...
    match tokens_after_item {
        ...
        // EPS
        _ => ParseResult::Ok(vec![parsed_item], tokens_after_item)
    }
}
...
fn parse_stmt_items<'a>(tokens: &'a[Tok]) -> ParseResult<'a, Stmt<'a>> {
    ...
    match tokens_after_items {
        ...
        [Tok{token: Token::Assign, line, ..}, rest..] => {
            let (parsed_expr, tokens_after_expr) = get_parsed!(parse_expr(rest));
            ParseResult::Ok(Stmt::Assignment{items: parsed_items, expr: Box::new(parsed_expr), data: AstData{line: line}}, tokens_after_expr)
        }
        // EPS
        [Tok{line, ..}, ..] => ParseResult::Ok(Stmt::Bare{items: parsed_items, data: AstData{line: line}}, tokens_after_items),
        //TODO: fix line number for last bare statement
        [] => ParseResult::Ok(Stmt::Bare{items: parsed_items, data: AstData{line: 0}}, tokens_after_items)
    }
}
...
fn parse_stmt<'a>(tokens: &'a[Tok]) -> ParseResult<'a, Stmt<'a>> {
    match tokens {
        ...
        [Tok{token: Token::If, line, ..}, rest..] => {
            ...
            ParseResult::Ok(Stmt::If{clauses: clauses, data: AstData{line: line}}, tokens_after_if)
        }
        ...
        [Tok{token: Token::While, line, ..}, rest..] => {
            ...
            ParseResult::Ok(Stmt::While{condition: Box::new(parsed_expr), statements: stmt_list, data: AstData{line: line}}, tokens_after_list)

        }
        // "return"
        [Tok{token: Token::Return, line, ..}, rest..] => ParseResult::Ok(Stmt::Return{data: AstData{line: line}}, rest),
        // <stmt-items>
...
// <if-statements>
fn parse_if_statements<'a>(tokens: &'a[Tok], expr: Expr<'a>) -> ParseResult<'a, Vec<IfClause<'a>>> {
    ...
    while my_toks.len() > 0 {
        ...
        match tok.token {
            ...
            Token::Else => {
                ...
                return ParseResult::Ok(clauses, tokens_after_list);
            }
            ...
            Token::End => {
                clauses.push(IfClause::If{condition: Box::new(my_expr), statements: statements});
                return ParseResult::Ok(clauses, &my_toks[1..])
            }
            // <stmt> <if-statements>
            _ => {
...
fn parse_block_statements<'a>(tokens: &'a[Tok]) -> ParseResult<'a, Vec<Stmt<'a>>> {
    ...
    while my_toks.len() > 0 {
        ...
        match tok.token {
            // "end"
            Token::End => {return ParseResult::Ok(statements, &my_toks[1..])}
...
pub fn parse_clip_statements<'a>(tokens: &'a[Tok]) -> ParseResult<'a, Vec<Stmt<'a>>> {
    ...
    while my_toks.len() > 0 {
        ...
        match tok.token {
            // "}"
            Token::CloseCurly => {return ParseResult::Ok(statements, &my_toks[1..])}
...
// <base-statements>
pub fn parse_base_statements<'a>(tokens: &'a[Tok], cur_statements: &'a mut Vec<Stmt<'a>>) -> ParseResult<'a, &'a Vec<Stmt<'a>>> {
    let mut my_toks = tokens;
    while my_toks.len() > 0 {
        let (parsed_stmt, tokens_after_stmt) = get_parsed!(parse_stmt(my_toks));
    ...
    ParseResult::Ok(cur_statements, my_toks)
}
